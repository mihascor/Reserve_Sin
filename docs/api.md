...
## Запланированный маршрут веб-истории

Для закрытой страницы `/app/` будет добавлен отдельный read-only маршрут под `/api/v1`. Его точное имя и JSON-формат ещё не реализованы. Он будет возвращать постраничный список неотменённых операций активных категорий; это правило не изменяет существующий `GET /api/v1/transactions`, используемый Android. Доступ к нему для браузера будет предоставляться только Caddy через `/app-api/*`: после Basic Auth Caddy добавит внутренний Bearer-токен.
