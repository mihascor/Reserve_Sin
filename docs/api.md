...
| `GET /api/v1/web-history` | Получить read-only историю для закрытой веб-страницы; параметры: `limit`, `cursor`. |
...
`GET /api/v1/web-history` — отдельный Bearer-защищённый read-only маршрут. Он возвращает постраничный список только неотменённых операций активных категорий, отсортированный от новых к старым по `occurred_at`, затем `id`. Архивная метка не скрывает операцию активной категории: возвращается имя сохранённой метки. Это правило не изменяет существующий `GET /api/v1/transactions`, используемый Android.

Параметр `limit` необязателен, по умолчанию `50`, допустим от `1` до `100`. `cursor` — значение непустого `next_cursor` с предыдущей страницы. Некорректный, отменённый или относящийся к архивной категории курсор даёт `400`.
