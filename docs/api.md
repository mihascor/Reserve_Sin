...
`income_rub` или `expense_rub` содержит положительное целое число рублей; другое поле равно `null`. `label_id`, `label_name`, `comment` и `next_cursor` также могут быть `null`. Веб-страница обращается к этому маршруту как `/app-api/api/v1/web-history`, а к `/app-api/api/v1/categories` и `/app-api/api/v1/labels` — для порядка столбцов и выбора метки. Caddy заменяет клиентский `Authorization` внутренним Bearer-заголовком.
...
`PATCH /transactions/{id}` принимает только поле `label_id` (ID активной метки или `null`). Он заменяет метку неотменённой операции без создания отдельной записи аудита; остальные поля операции этим методом не изменяются. Несуществующая или архивная метка даёт `400`, отменённая операция — `409`.
