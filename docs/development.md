...
В репозитории есть Gradle-конфигурация Android и `server/go.mod`. Для Android нужны JDK 17, Android SDK API 36 и Gradle; для Go — Go 1.22 или новее. Gradle Wrapper не добавлен, поскольку инструменты Gradle и Java недоступны в текущем окружении для его безопасной генерации.
...
- `cd server && go test ./...` — тест HTTP-маршрута `/health`;
- `cd server && go run ./cmd/reserve-server` — локальный запуск сервера на `127.0.0.1:8080`.
...
После добавления базы, API и Android-функций необходимо дополнить документ командами миграций и статического анализа. Перед публикацией следует проверять соответствие API, работу offline-сценария, повторную синхронизацию, отмену операций, права доступа и резервное восстановление.
