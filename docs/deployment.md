...
```bash
ssh -i ~/.ssh/server_193_124_91_76 root@193.124.91.76
```
...
Go API слушает только `127.0.0.1:8080`. Caddy публикует `https://reserve-sin.duckdns.org:8443` и проксирует запросы к нему. Для этого сайта разрешены HTTPS по HTTP/1.1 и HTTP/2; HTTP/3 отключён из-за подтверждённой ошибки Chrome `ERR_QUIC_PROTOCOL_ERROR`. Снаружи нужны порты `80` и `8443`; порт `443` не используется Reserve_Sin и свободен для другого проекта. Существующие порты AmneziaVPN не изменяются. Права должны ограничивать доступ посторонних пользователей к env-файлу, базе и резервным копиям.
