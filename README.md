...
В репозитории есть начальная local-first основа Android-приложения и Go-сервер. Android хранит локальные категории, метки и операции в Room, позволяет создавать, изменять и архивировать категории, а также создавать одиночные и групповые операции со статусом `PENDING`. Главный Compose-экран получает данные через `ViewModel` и `Repository`. Метки в интерфейсе, история, отмена, настройки, импорт и синхронизация с сервером пока не реализованы. Сервер открывает SQLite, автоматически применяет начальную SQL-миграцию, предоставляет Bearer-защищённый REST API и `GET /health` с проверкой базы. Остальные описанные ниже возможности являются требованиями [ТЗ](docs/ТЗ.md).
