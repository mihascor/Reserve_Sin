...
В репозитории есть начальная local-first основа Android-приложения и Go-сервер. Android хранит локальные категории, метки и операции в Room, выводит главный Compose-экран через `ViewModel` и `Repository`, но пока не позволяет редактировать данные и не синхронизируется с сервером. Сервер открывает SQLite, автоматически применяет начальную SQL-миграцию, предоставляет Bearer-защищённый REST API и `GET /health` с проверкой базы. Остальные описанные ниже возможности являются требованиями [ТЗ](docs/ТЗ.md).
